...
void moverBody(void *arg)
{
  ...
  printf("%5d ms: T%02d movendo %2d blocos entre bloco %3d e bloco %3d\n",
  ...
  for (i = 0; i < blocksPerTask; i++)
  {
    ...
    printf("%5d ms: T%02d lendo bloco %3d\n", systime(), task_id(), block_orig);
    if (disk_block_read(block_orig, buffer1) == 0)
      printf("%5d ms: T%02d leu   bloco %3d\n", systime(), task_id(), block_orig);
    else
      printf("%5d ms: T%02d erro ao ler bloco %3d\n", systime(), task_id(), block_orig);
    ...
    printf("%5d ms: T%02d bloco %3d tem: [", systime(), task_id(), block_orig);
    for (j = 0; j < blocksize; j++)
      printf("%c", buffer1[j]);
    printf("]\n");

    // mostra o conteudo do bloco b2
    printf("%5d ms: T%02d bloco %3d tem: [", systime(), task_id(), block_dest);
    for (j = 0; j < blocksize; j++)
      printf("%c", buffer2[j]);
    printf("]\n");

    // escreve o bloco b1 no disco
    printf("%5d ms: T%02d escrevendo bloco %3d\n", systime(), task_id(), block_dest);
    if (disk_block_write(block_dest, buffer1) == 0)
      printf("%5d ms: T%02d escreveu   bloco %3d\n", systime(), task_id(), block_dest);
    else
      printf("%5d ms: T%02d erro ao escrever bloco %3d\n", systime(), task_id(), block_dest);
...
int main(int argc, char *argv[])
{
  ...
  for (i = 0; i < NUMTASKS; i++)
    task_init(&mover[i], moverBody, (long *)i);
