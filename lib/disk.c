...
#define DISK_NAME "lib/disk.dat" // arquivo com o conteúdo do disco
...
static void disk_settimer()
{
  ...
  time_ms = abs(disk.next_block - disk.prev_block) * (disk.delay_max - disk.delay_min) / disk.numblocks + disk.delay_min + rand() % (disk.delay_max - disk.delay_min) / 10;
